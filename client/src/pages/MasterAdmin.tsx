...
const organizationCategories = {
  upperCategories: ["대학본부", "학사부서", "인문대학", "사회과학대학", "자연과학대학", "공과대학", "경영대학", "의과대학", "대학원", "연구기관"],
  lowerCategories: {
    "대학본부": ["총장실", "기획처", "교무처", "학생처"],
    "학사부서": ["입학처", "교육혁신원", "국제교류원", "취업진로센터"],
    "인문대학": ["국어국문학과", "영어영문학과", "사학과", "철학과"],
    "사회과학대학": ["정치외교학과", "행정학과", "사회학과", "심리학과"],
    "자연과학대학": ["수학과", "물리학과", "화학과", "생명과학과"],
    "공과대학": ["컴퓨터공학과", "전자공학과", "기계공학과", "건축학과"],
    "경영대학": ["경영학과", "회계학과", "경제학과"],
    "의과대학": ["의학과", "간호학과", "약학과", "치의학과"],
    "대학원": ["일반대학원", "특수대학원", "전문대학원", "국제대학원"],
    "연구기관": ["중앙연구소", "인문사회연구소", "의학연구소", "공학연구소"]
  },
  detailCategories: {
    "총장실": ["총장비서실", "대외협력팀", "홍보팀"],
    "기획처": ["기획예산팀", "평가관리팀", "IR팀"],
    "교무처": ["교무팀", "학사관리팀", "수업관리팀"],
    "학생처": ["학생지원팀"],
    "입학처": ["입학사정팀", "입학전형팀", "입학홍보팀"],
    "교육혁신원": ["교수학습지원센터", "교육과정혁신팀", "이러닝지원센터"],
    "국제교류원": ["국제협력팀", "외국인학생지원팀", "교환학생팀"],
    "취업진로센터": ["취업지원팀"],
    "국어국문학과": ["현대문학전공", "고전문학전공", "국어학전공"],
    "영어영문학과": ["영문학전공", "영어학전공", "영어교육전공"],
    "사학과": ["한국사전공", "동양사전공", "서양사전공"],
    "철학과": ["동양철학전공"],
    "정치외교학과": ["정치학전공", "외교학전공", "국제관계전공"],
    "행정학과": ["일반행정전공", "지방행정전공", "정책학전공"],
    "사회학과": ["일반사회학전공", "응용사회학전공"],
    "심리학과": ["임상심리전공", "인지심리전공"],
    "수학과": ["순수수학전공", "응용수학전공", "통계학전공"],
    "물리학과": ["이론물리전공", "실험물리전공", "응용물리전공"],
    "화학과": ["유기화학전공", "무기화학전공", "물리화학전공"],
    "생명과학과": ["분자생물학전공"],
    "컴퓨터공학과": ["소프트웨어공학전공", "인공지능전공", "데이터사이언스전공"],
    "전자공학과": ["반도체공학전공", "통신공학전공", "제어공학전공"],
    "기계공학과": ["설계공학전공", "열유체공학전공", "재료공학전공"],
    "건축학과": ["건축설계전공"],
    "경영학과": ["경영전략전공", "마케팅전공", "재무관리전공", "인사조직전공"],
    "회계학과": ["재무회계전공", "관리회계전공", "세무회계전공"],
    "경제학과": ["미시경제전공", "거시경제전공", "계량경제전공"],
    "의학과": ["내과학전공", "외과학전공", "소아과학전공"],
    "간호학과": ["성인간호전공", "아동간호전공", "지역간호전공"],
    "약학과": ["임상약학전공", "제약학전공"],
    "치의학과": ["구강내과전공", "구강외과전공"],
    "일반대학원": ["박사과정", "석사과정", "석박통합과정"],
    "특수대학원": ["경영대학원", "교육대학원", "행정대학원"],
    "전문대학원": ["의학전문대학원", "법학전문대학원", "경영전문대학원"],
    "국제대학원": ["국제관계전공"],
    "중앙연구소": ["기초과학연구소", "응용과학연구소", "산업기술연구소"],
    "인문사회연구소": ["인문학연구소", "사회과학연구소", "교육연구소"],
    "의학연구소": ["임상의학연구소", "기초의학연구소"],
    "공학연구소": ["신소재연구소", "환경공학연구소"]
  }
};
...
export default function MasterAdmin() {
  ...
  // 상태 변수
  const [userUpperCategory, setUserUpperCategory] = useState('all');
  const [userLowerCategory, setUserLowerCategory] = useState('all');
  const [userDetailCategory, setUserDetailCategory] = useState('all');
  ...
  const handleUserCategoryChange = (category: 'upper' | 'lower' | 'detail', value: string) => {
    if (category === 'upper') {
      setUserUpperCategory(value);
      setUserLowerCategory('all');
      setUserDetailCategory('all');
    } else if (category === 'lower') {
      setUserLowerCategory(value);
      setUserDetailCategory('all');
    } else {
      setUserDetailCategory(value);
    }
    setHasSearched(true);
    setUserCurrentPage(1);
  };
  ...
  const filteredUsers = useMemo(() => {
    ...
    // 카테고리 필터링
    if (userUpperCategory !== 'all') {
      filtered = filtered.filter(user => user.upperCategory === userUpperCategory);
    }
    if (userLowerCategory !== 'all') {
      filtered = filtered.filter(user => user.lowerCategory === userLowerCategory);
    }
    if (userDetailCategory !== 'all') {
      filtered = filtered.filter(user => user.detailCategory === userDetailCategory);
    }
    ...
    if (userSortField) {
      filtered.sort((a, b) => {
        let aValue = a[userSortField as keyof User] || '';
        let bValue = b[userSortField as keyof User] || '';
    ...
    return filtered;
  }, [users, userSearchQuery, userUpperCategory, userLowerCategory, userDetailCategory, userSortField, userSortDirection]);
  ...
  // 총 페이지 수 계산
  const totalUserPages = Math.ceil((filteredUsers?.length || 0) / usersPerPage);
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="users" className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">사용자 검색 및 관리</h3>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    ...
                    <Select value={userUpperCategory} onValueChange={(value) => handleUserCategoryChange('upper', value)}>
                      <SelectTrigger className="h-11">
                        <SelectValue placeholder="전체" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">전체</SelectItem>
                        {organizationCategories.upperCategories.map((category) => (
                  ...
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleUserCategoryChange('lower', value)}
                      disabled={userUpperCategory === 'all'}
                    >
                      <SelectTrigger className="h-11">
                        <SelectValue placeholder="전체" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">전체</SelectItem>
                        {userUpperCategory !== 'all' && organizationCategories.lowerCategories[userUpperCategory]?.map((category) => (
                          <SelectItem key={category} value={category}>
                            {category}
                          </SelectItem>
                        ))}
                  ...
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleUserCategoryChange('detail', value)}
                      disabled={userLowerCategory === 'all'}
                    >
                      <SelectTrigger className="h-11">
                        <SelectValue placeholder="전체" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">전체</SelectItem>
                        {userLowerCategory !== 'all' && organizationCategories.detailCategories[userLowerCategory]?.map((category) => (
                          <SelectItem key={category} value={category}>
                            {category}
                          </SelectItem>
                        ))}
